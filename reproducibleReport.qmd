...
#list_of_dataframes <- list(getDataset(635))
```
...
write_rds(mosqDF, file = here("mosq.rds"))
write_rds(mosqDFcomb, file = here("mosq_comb.rds"))
```

```{r, echo = FALSE}
if(DL == FALSE){
  mosqDF <- read_rds(file = here("mosq.rds"))
  mosqDFcomb <- read_rds(file = here("mosq_comb.rds"))
}
```
...
mosqMonth$totalMos_sqrt <- sqrt(mosqMonth$totalMos)
mosqMonth$totalMos_log1p <- log1p(mosqMonth$totalMos)

mosqMod <- lm(totalMos ~ time + totalMos_lag1 + month,
              data = mosqMonth)

mosqMod_sqrt <- lm(totalMos_sqrt ~ time + totalMos_lag1 + month,
              data = mosqMonth)

mosqMod_log1p <- lm(totalMos_log1p ~ time + totalMos_lag1 + month,
              data = mosqMonth)

mosqMod_pois <- glm(totalMos ~ time + totalMos_lag1 + month,
               family = "poisson",
               data = mosqMonth)

mosq_gam <- gam(totalMos ~ s(time),
                family = "poisson",
                data = mosqMonth)
...
plot(predict(mosqMod_sqrt), mosqMod_sqrt$residuals)
plot(predict(mosqMod_pois), mosqMod_pois$residuals)
plot(predict(mosq_gam), mosq_gam$residuals)
...
mosqMonth$pred_sqrt <- predict(mosqMod_sqrt)^2
mosqMonth$pred_log1p <- exp(predict(mosqMod_log1p))
mosqMonth$pred_pois <- exp(predict(mosqMod_pois))
mosqMonth$pred_gam <- exp(predict(mosq_gam))
...
ggplot(data = mosqMonth, aes(x = date, y = totalMos))+
  geom_line(aes(color = "actual"))+
  geom_point(aes(color = "actual"))+
  geom_line(aes(y = pred_sqrt, color = "predicted"), linewidth = 1.5,
            alpha = 0.3)+
  scale_color_manual(
    name = "Color",
    values = colors2)+
...
ggplot(data = mosqMonth, aes(x = date, y = totalMos))+
  geom_line(aes(color = "actual"))+
  geom_point(aes(color = "actual"))+
  geom_line(aes(y = pred_log1p, color = "predicted"), linewidth = 1.5,
            alpha = 0.3)+
  scale_color_manual(
    name = "Color",
    values = colors2)+
...
ggplot(data = mosqMonth, aes(x = date, y = totalMos))+
  geom_line(aes(color = "actual"))+
  geom_point(aes(color = "actual"))+
  geom_line(aes(y = pred_pois, color = "predicted"), linewidth = 1.5,
            alpha = 0.3)+
  scale_color_manual(
    name = "Color",
    values = colors2)+
...
ggplot(data = mosqMonth, aes(x = date, y = totalMos))+
  geom_line(aes(color = "actual"))+
  geom_point(aes(color = "actual"))+
  geom_line(aes(y = pred_gam, color = "predicted"), linewidth = 1.5,
            alpha = 0.3)+
  scale_color_manual(
    name = "Color",
    values = colors2)+
